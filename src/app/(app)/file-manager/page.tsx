...
export default function FileManagerPage() {
    ...
    const { loggedInUser, users, loadingUser } = useSettings();
    ...
    const getCreatorName = (item: FileItem) => {
        const creatorId = item.metadata?.customMetadata?.creatorId;
                                ...
                                {showLoading ? (
                                    ...
                                    sortedItems.map((item) => (
                                        <TableRow key={item.name}>
                                            ...
                                            <TableCell>{item.type === 'file' ? getCreatorName(item) : '-'}</TableCell>
                                    ...
                                    <TableRow><TableCell colSpan={5} className="h-24 text-center text-muted-foreground">{searchTerm ? `No results for "${searchTerm}"` : 'This folder is empty.'}</TableCell></TableRow>
                                )}
                            </TableBody>
                        </Table>
                    </div>
